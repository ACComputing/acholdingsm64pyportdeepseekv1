...
# Movement speed (Famicom-style)
MOVE_SPEED = 0.08
ROT_SPEED = 0.02
CAMERA_HEIGHT = 2.0
CAMERA_DISTANCE = 5.0

# Colors (SM64 Peach's Castle style)
...
def create_castle():
    ...
    # --- Main Roof (pointed spire) ---
    roof_height = 2.5
    roof_vertices = [
        (-kw/2, kh, -kd/2), ( kw/2, kh, -kd/2), ( kw/2, kh,  kd/2), (-kw/2, kh,  kd/2),
        (0, kh + roof_height, 0)
    ]
    ...
    door_vertices = [
        (-door_w/2, 0, -kd/2), ( door_w/2, 0, -kd/2),
    ...
    # --- Left Tower (cylindrical + conical roof) ---
    tower_pos = (-3.5, 1.0, -1.5)
    tower_radius, tower_height = 1.2, 4.0
    segments = 10
    tower_vertices = []
    ...
    for i in range(segments):
        j = (i+1)%segments
        tower_faces.append([2*i+1, 2*j+1, apex_idx])
    ...
    for i in range(segments):
        j = (i+1)%segments
        ct_faces.append([2*i+1, 2*j+1, apex_idx_ct])
...
def game_loop(screen):
    ...
    while running:
        ...
        # Forward/back relative to player angle
        ...
        # Camera: position = player + offset (behind and above)
        # For angle 0 (facing -Z), behind is +Z, so offset = (0, CAMERA_HEIGHT, CAMERA_DISTANCE)
        cam_offset = (CAMERA_DISTANCE * math.sin(player_angle),
                      CAMERA_HEIGHT,
                      CAMERA_DISTANCE * math.cos(player_angle))
        cam_pos = vec_add(player.position, cam_offset)   # FIXED: was vec_sub
...
def main():
    ...
    pygame.display.set_caption("Super Mario 64 - Peach's Castle")
